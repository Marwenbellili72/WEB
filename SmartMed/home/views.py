...
def segment_image(request):
    """
    Process segmentation and 3D visualization
    """
    ...
    # Cleanup folders before processing
    cleanup_folders(input_folder, output_folder)
    ...
    try:
        ...
        # Path to segmented image (using hip_left as in original code)
        segmented_file = os.path.join(output_folder, 'heart.nii.gz')
        
        # Load segmented image
        img = nib.load(segmented_file)
        data = img.get_fdata()
        print("Segmented image loaded successfully.")
        ...
        # Threshold and surface rendering
        thresholded = grid.threshold(0.5, scalars="values")
        surface = thresholded.contour(isosurfaces=[0.5])
        
        # Optional: Save visualization screenshot
        plotter = pv.Plotter(off_screen=True)
        plotter.add_mesh(surface, color="cyan", opacity=0.7)
        plotter.show(screenshot=os.path.join(output_folder, 'segmentation_visualization.png'))
        
        return JsonResponse({
            'status': 'success', 
            'message': 'Segmentation completed',
            'segmented_file_path': '/media/output/heart.nii.gz'  # Return the path for the download link
        })
